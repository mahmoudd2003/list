...
query_text = st.text_input("اكتب ما تريد البحث عنه", placeholder="مثال: مطاعم برجر في الرياض")
...
with tab2:
    ...
    if st.button("انشر كمسودة"):
        ...
        if not items:
            st.error("⚠️ لا يوجد نتائج، جلبها أولًا من تبويب المعاينة")
        elif not wp_base_url or not wp_user or not wp_app_pass:
            st.error("⚠️ أدخل بيانات ووردبريس (URL + Username + App Password)")
        else:
            try:
