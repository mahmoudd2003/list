...
with st.sidebar:
    ...
    st.caption("يفضّل وضع المفاتيح في Secrets داخل Streamlit Cloud.")
...
if st.button("جلب النتائج"):
    if not google_api_key:
        st.error("⚠️ أدخل Google API Key")
    elif not final_query:
        st.error("⚠️ أدخل استعلام للبحث")
    else:
        try:
            ...
            st.success(f"✅ تم الجلب. عدد النتائج بعد الفلترة: {len(items)}")
            ...
            st.error(f"❌ خطأ أثناء الجلب: {e}")

st.markdown("---")

# --------------------------
# النشر إلى ووردبريس (يغلب يظهر دومًا)
# --------------------------
st.subheader("📝 نشر كمسودة في ووردبريس")
draft_title = st.text_input("عنوان المقال", value=f"{final_query or 'قائمة مطاعم'} — محدّث")

if st.button("انشر كمسودة"):
    items = st.session_state.get("items", [])
    if not items:
        st.error("⚠️ لا يوجد نتائج في الذاكرة. الرجاء جلب النتائج أولًا.")
    elif not wp_base_url or not wp_user or not wp_app_pass:
        st.error("⚠️ أدخل إعدادات ووردبريس كاملة (URL + Username + App Password).")
    else:
        try:
            ...
            if res.get("link"):
                st.markdown(f"[رابط المسودة في ووردبريس]({res['link']})")
        except Exception as e:
